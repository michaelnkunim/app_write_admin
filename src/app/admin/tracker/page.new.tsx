...
export default function TaskTracker() {
  ...
  const handleAddSubtaskToTask = async (taskId: string) => {
    ...
    try {
      ...
      // Get the current date and time for the default due date
      const now = new Date();
  ...
  const handleSprintStatusChange = async (sprintId: string, newStatus: 'planning' | 'active' | 'completed') => {
    try {
      // Check if we're activating a new sprint
      if (newStatus === 'active') {
        ...
        if (currentActive && currentActive.id !== sprintId) {
          if (!confirm(`There is already an active sprint (${currentActive.name}). Do you want to complete it and activate this one instead?`)) {
            return;
          }
          
          // Complete the current active sprint
          await updateDoc(doc(db, 'sprints', currentActive.id), {
            status: 'completed'
          });
        }
      }
      ...
      } else if (newStatus !== 'active' && activeSprint === sprintId) {
          ...
          <>
            {calendarView ? (
              ...
              <div>
                ...
                <div className="flex overflow-x-auto pb-4 -mx-4 px-4 space-x-4 min-w-full">
                  {/* Backlog column */}
                  <div 
                    className="border rounded-lg bg-card flex-shrink-0 w-80"
                    onDragOver={(e) => {
                      e.preventDefault();
                            ...
                            {task.assignedToName && (
                              <span className="flex items-center gap-1">
                                {task.assignedToPhotoURL ? (
                                  <img src={task.assignedToPhotoURL} alt={task.assignedToName} className="w-4 h-4 rounded-full" />
                                ) : (
                                  <div className="w-4 h-4 rounded-full bg-primary/10 flex items-center justify-center text-[10px] text-primary">
                      ...
                      {getTasksByStatus('completed').length > 0 ? getTasksByStatus('completed').map(task => (
                        ...
                        >
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <div className={`px-2 py-0.5 rounded-full text-xs bg-gray-100 text-gray-600`}>
                              Completed
                            </div>
                          </div>
                          <div className="flex justify-between items-center gap-2 text-xs text-muted-foreground mb-2">
